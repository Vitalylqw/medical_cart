...
- Прогресс: 95%
- Сделано: каркас, конфиг, хранилище, аудио‑пайплайн, провайдеры, роутер, бот, кэш, скрипты, unit-тесты (core/config, core/storage, transcription/router), скрипты run_tests.bat и run_coverage.bat, покрытие кода 59%
- Осталось: обновить пользовательскую/техдокументацию (провайдеры, тесты, CI), завести CI (ruff + pytest-cov) и адаптировать конфиг Ruff к новой схеме `lint` (частично выполнено: конфиг обновлен до `[tool.ruff.lint]`).
